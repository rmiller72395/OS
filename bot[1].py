...
# ── Model routing (v5.0): centralized model_routing.json; fallbacks from config ─
try:
    from model_routing import (
        load_routing,
        resolve_model,
        get_fallback_chain,
        is_worker_paid_fallback_gated,
        get_routing_summary,
        validate_routing,
    )
    _MODEL_ROUTING_AVAILABLE = True
except Exception as e:
    logging.warning("Model routing not available: %s", e)
    _MODEL_ROUTING_AVAILABLE = False

# Legacy constants: used when routing unavailable or for tier/fail-up logic (routing_map)
...
# Model tiers (Lowest Viable Model routing; used with routing_map / model_hint)
...
def _role_to_layer(role: str) -> Optional[str]:
    """Map role name to model_routing layer (CEO, CFO, CISO, DIRECTOR, MANAGER, PROCESS_OPTIMIZER, WORKER_EXECUTION)."""
    if not role:
        return None
    r = str(role).strip().upper()
    if r == "CEO" or r.startswith("CEO_"): return "CEO"
    if r == "CFO": return "CFO"
    if r == "CISO": return "CISO"
    if r == "DIRECTOR": return "DIRECTOR"
    if r == "PROCESS_OPTIMIZER": return "PROCESS_OPTIMIZER"
    if r.startswith("MANAGER_"): return "MANAGER"
    if r.startswith("WORKER_"): return "WORKER_EXECUTION"
    return None


def _provider_from_litellm_model(litellm_model: str) -> str:
    """Extract provider from provider/model string."""
    if "/" in litellm_model:
        return litellm_model.split("/", 1)[0].strip().lower()
    return ""


def _candidates_for_layer_or_override(layer: Optional[str], model_override: Optional[str]) -> List[str]:
    """Build ordered list of litellm model strings (primary + fallbacks). Uses routing when available."""
    try:
        if model_override:
            # Per-work routing override (e.g. from routing_map): primary = override, then layer fallbacks or legacy
            if _MODEL_ROUTING_AVAILABLE and layer:
                chain = get_fallback_chain(layer)
                rest = [c["litellm_model"] for c in chain if c.get("litellm_model") and c["litellm_model"] != model_override]
                return [model_override] + rest
            return [model_override] + MODEL_FALLBACK_CHAINS.get(model_override, [])
        if _MODEL_ROUTING_AVAILABLE and layer:
            chain = get_fallback_chain(layer)
            return [c["litellm_model"] for c in chain if c.get("litellm_model")]
    except Exception as e:
        logging.warning("Model routing fallback: %s", e)
    return [model_override] if model_override else [MODEL_DIRECTOR]
...
def parse_mission_funding(raw_cmd: str, *, default_threshold_usd: float = 0.0) -> Tuple[Dict[str, Any], str]:
    ...
    while i < len(lines):
        ...
        if key in ("PROJECT", "CASH_BUDGET_USD", "OWNER_APPROVAL_THRESHOLD_USD", "ALLOWED_TOOLS", "TICKET_ID", "TASK", "CFO_APPROVED_WORKER_PAID"):
            header_seen = True

        if key == "CFO_APPROVED_WORKER_PAID":
            funding["cfo_approved_worker_paid_fallback"] = str(val or "").strip().lower() in ("1", "true", "yes")
            i += 1
            continue
        if key == "TICKET_ID":
...
def _ensure_run_grant_if_needed(trace: Dict[str, Any]) -> None:
    """v5.0: For ad-hoc missions (run_id set, no ticket_id), create a run-scoped tool grant (READ-ONLY only, explicit tools/scopes, no wildcards)."""
    run_id = trace.get("run_id")
    if not run_id or trace.get("ticket_id"):
        return
    try:
        from skills.tool_grants import get_tool_grant_store, ToolGrant
        from skills.tool_registry import get_tool_registry, STARTER_READONLY_TOOL_NAMES, STARTER_READONLY_SCOPES
        ...
        reg.ensure_schema()
        # Auto-grant: ONLY explicit starter read-only tools and scopes (no wildcards)
        allowed_tools = [t.strip().upper() for t in STARTER_READONLY_TOOL_NAMES if t.strip()]
        allowed_scopes = [s.strip() for s in STARTER_READONLY_SCOPES if s.strip()]
        if not allowed_tools or not allowed_scopes:
            return
        now = _utcnow_iso()
        g = ToolGrant(
            ...
            reason="auto run-scoped grant (read-only starter set only)",
            created_at=now,
        )
...
async def get_sys_status() -> str:
    ...
    routing_line = ""
    if _MODEL_ROUTING_AVAILABLE:
        try:
            summary = get_routing_summary()
            if summary.get("layers") and "error" not in summary:
                layers = list(summary["layers"].keys())[:5]
                routing_line = f" | routing={','.join(layers)}"
        except Exception:
            pass
    return (
        f"[${snap['spend']:.4f}/${snap['limit']:.2f} aust=${snap['austerity']:.2f} | "
        f"{f_str} | fly={len(_inflight_missions)}/{MAX_CONCURRENT_MISSIONS} | "
        f"CPU={cpu:.0f}% RAM={mem.available/(1024**3):.1f}GB | {last_str}{routing_line}]"
    )
...
async def call_agent(role: str, mission_id: str, system_msg: str, user_msg: str,
                     model: Optional[str] = None, trace: Optional[Dict[str, Any]] = None,
                     deadline: Optional[float] = None, *,
                     layer: Optional[str] = None, model_override: Optional[str] = None) -> str:
    # Resolve layer from role if not provided (v5.0 centralized routing)
    resolved_layer = layer or _role_to_layer(role) or "CEO"
    candidates = _candidates_for_layer_or_override(resolved_layer, model_override or model)

    # SAFE_MODE: forbid paid model calls for any layer; for workers allow local only
    if SAFE_MODE:
        paid = frozenset({"anthropic", "openai", "google"})
        viable_safe = [m for m in candidates if _provider_from_litellm_model(m) not in paid]
        if not viable_safe:
            return f"[SYSTEM_ERROR: {role} SAFE_MODE] [MISSION_ID: {mission_id}]"
        candidates = viable_safe

    if DISABLE_PAID_CALLS:
        return f"[SYSTEM_ERROR: {role} kill-switch] [MISSION_ID: {mission_id}]"

    # CFO-gated worker paid fallback: if WORKER_EXECUTION has paid in chain, require approval or restrict to local only
    if resolved_layer == "WORKER_EXECUTION" and is_worker_paid_fallback_gated():
        funding = (trace or {}).get("funding") or {}
        if not funding.get("cfo_approved_worker_paid_fallback"):
            paid = frozenset({"anthropic", "openai", "google"})
            local_only = [m for m in candidates if _provider_from_litellm_model(m) not in paid]
            if not local_only:
                return (
                    f"[SYSTEM_ERROR: {role} worker paid fallback requires CFO approval] [MISSION_ID: {mission_id}] "
                    "Set CFO_APPROVED_WORKER_PAID=1 in mission funding or approve via Discord."
                )
    ...
    for attempt_index, candidate in enumerate(viable):
        actual_model = candidate
        provider = _provider_from_litellm_model(candidate)
        ...
        # Observability: log model chosen (run graph)
        if _TRACING_AVAILABLE and trace and trace.get("run_id") and _tracing_record_event:
            try:
                asyncio.create_task(_tracing_record_event(
                    trace["run_id"], "model_chosen", f"{role} -> {actual_model}",
                    "info", {"layer": resolved_layer, "provider": provider, "model": actual_model,
                             "attempt_index": attempt_index, "reason": "primary" if attempt_index == 0 else "fallback",
                             "model_call": True}
                ))
            except Exception:
                pass
...
class SovereignBot(discord.Client):
    ...
    async def on_ready(self) -> None:
        ...
        try:
            ...
            async with _cfg.lock:
                cfg = _cfg.snapshot()
            bootstrap_builtin_tools(reg, cfg)
        except Exception as e:
            logging.warning("Tool Registry bootstrap on startup: %s", e)
    ...
    async def _worker_loop(self) -> None:
        try:
            while not _draining:
                ...
                _worker_default = MODEL_DIRECTOR
                if _MODEL_ROUTING_AVAILABLE:
                    _wc = get_fallback_chain("WORKER_EXECUTION")
                    if _wc:
                        _worker_default = _wc[0]["litellm_model"]
                model_hint = str(item.get("model_hint") or "").strip() or _worker_default
                ...
                result = await call_agent(
                    ...
                    trace=mtrace,
                    deadline=None,
                    model_override=model_hint if model_hint else None,
                )
    ...
    async def _try_emit_mission_report(self, mission_id: str) -> None:
        ...
        try:
            ...
            report_text = await call_agent(
                ...
                eval_sys,
                eval_user,
                None, trace=None, deadline=None, layer="CEO",
            )
    ...
    async def _board_manager_plan(self, message: discord.Message, trace: Dict[str, Any], fanout: int) -> List[WorkOrder]:
        ...
        mgr_calls = []
        _mgr_default = MODEL_DIRECTOR
        if _MODEL_ROUTING_AVAILABLE:
            _mc = get_fallback_chain("MANAGER")
            if _mc:
                _mgr_default = _mc[0]["litellm_model"]
        for i in range(fanout):
            pseudo_id = f"MANAGER_PLAN_{i+1}"
            mdl = str(routing_map.get(pseudo_id) or "").strip() or _mgr_default
            mdl = _fail_up_from_model(mdl)
            mgr_calls.append(
                call_agent(
                    f"MANAGER_{i+1}",
    ...
    async def _run_mission(self, message: discord.Message, trace: Dict[str, Any], deadline: float) -> None:
        ...
        async with message.channel.typing():
            ...
            t2_res = await call_agent(
                ...
                director_user_msg, None, trace, deadline, layer="DIRECTOR"
            )
            ...
            gate_results = await asyncio.gather(
                call_agent(
                    ...
                    fenced_t2, None, trace, deadline, layer="CISO",
                ),
                call_agent(
                    "CFO", mission_id,
                    _cfo_sop(mission_id),
                    fenced_t2, None, trace, deadline, layer="CFO",
                ),
            ...
            if routing_targets:
                ...
                opt_out = await call_agent(
                    ...
                    opt_user,
                    None, trace, deadline, layer="PROCESS_OPTIMIZER",
                )
                trace["t_opt"] = opt_out
            ...
            ceo_res = await call_agent(
                ...
                fenced_audit, None, trace, deadline, layer="CEO",
            )
