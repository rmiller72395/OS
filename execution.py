...
# ---------------------------------------------------------------------------
# Starter tools (v5.0 safe read-only set)
# ---------------------------------------------------------------------------

class TimeNowSkill(BaseSkill):
    """Returns UTC time and local time (America/New_York). Read-only."""

    def __init__(self) -> None:
        super().__init__(
            name="time_now",
            description="Returns UTC time and local time (America/New_York). Params: none.",
            version="1.0.0",
            access_level=AccessLevel.GLOBAL,
            idempotent=True,
        )
    ...
    async def _execute_impl(self, params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
        ...
        try:
            ...
            local_iso = utc.isoformat()
        out = {"utc_iso": utc.isoformat(), "local_iso": local_iso}
        return ActionResult("SUCCESS", f"utc={utc.isoformat()}", out)


class UuidNewSkill(BaseSkill):
    """Generate a UUID. Read-only."""

    def __init__(self) -> None:
        super().__init__(
            name="uuid_new",
            description="Generate a UUID. Params: none.",
            version="1.0.0",
            access_level=AccessLevel.GLOBAL,
            idempotent=False,
        )
    ...
    async def _execute_impl(self, params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
        uid = uuid.uuid4().hex
        return ActionResult("SUCCESS", uid, {"uuid": uid})


class JsonValidateSkill(BaseSkill):
    """Validate JSON string to dict; optional JSON schema. Read-only."""

    def __init__(self) -> None:
        super().__init__(
            name="json_validate",
            description="Validate JSON string to dict; optional schema. Params: json_str, schema (optional).",
            version="1.0.0",
            access_level=AccessLevel.GLOBAL,
            idempotent=True,
        )
    ...
    def validate(self, params: Dict[str, Any]) -> None:
        super().validate(params)
        if not params.get("json_str") and "json_str" in params:
            raise ValidationError("json_str required")

    async def _execute_impl(self, params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
        raw = params.get("json_str")
        if raw is None:
            return ActionResult("FAIL", "json_str required", None)
        if not isinstance(raw, str):
            return ActionResult("FAIL", "json_str must be string", None)
        if len(raw) > 500_000:
            return ActionResult("FAIL", "json_str too long", None)
        try:
            obj = json.loads(raw)
        except json.JSONDecodeError as e:
            return ActionResult("FAIL", f"invalid JSON: {e!s}", None)
        ...
        if schema is not None and isinstance(schema, dict):
            try:
                ...
                return ActionResult("FAIL", "schema validation requires jsonschema", None)
            except Exception as e:
                return ActionResult("FAIL", f"schema validation failed: {e!s}", None)
        return ActionResult("SUCCESS", "valid", {"parsed": obj, "valid": True})


class HttpGetJsonReadonlySkill(BaseSkill):
    """HTTP GET to fetch JSON from allowlisted domains only. Enforces allowlist, max bytes, Content-Type JSON."""

    def __init__(self) -> None:
        super().__init__(
            name="http_get_json_readonly",
            description="HTTP GET to fetch JSON from allowlisted domains only. Params: url.",
            version="1.0.0",
            access_level=AccessLevel.RESTRICTED,
            idempotent=True,
        )
    ...
    async def _execute_impl(self, params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
        from skills.public_api_allowlist import get_allowlist_config, is_url_allowed
        url = (params.get("url") or "").strip()
        if not url:
            return ActionResult("FAIL", "url required", None)
        allowed, reason = is_url_allowed(url)
        if not allowed:
            return ActionResult("FAIL", reason, None)
        cfg = get_allowlist_config()
        max_bytes = int(cfg.get("http_max_bytes") or 1_000_000)
        max_redirects = int(cfg.get("http_max_redirects") or 0)
        timeout_s = min(float(context.timeout_seconds), float(cfg.get("http_max_timeout_s") or 60))
        if not _HAS_AIOHTTP:
            return ActionResult("FAIL", "aiohttp not installed", None)
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if resp.status != 200:
                        return ActionResult("FAIL", f"HTTP {resp.status}", None)
                    ct = (resp.headers.get("Content-Type") or "").lower()
                    if "application/json" not in ct and "+json" not in ct:
                        return ActionResult("FAIL", "Content-Type must be application/json or +json", None)
                    body = await resp.read()
                    if len(body) > max_bytes:
                        return ActionResult("FAIL", f"response exceeds {max_bytes} bytes", None)
                    data = json.loads(body.decode("utf-8", errors="replace"))
                    preview = _redact_summary(json.dumps(data)[:500], 500)
                    return ActionResult("SUCCESS", f"len={len(body)}", {"json": data, "body_preview": preview})
        except asyncio.TimeoutError:
            return ActionResult("TIMEOUT", f"timeout after {timeout_s}s", None)
        except Exception as e:
            return ActionResult("FAIL", _redact_summary(str(e)[:500], 500), None)


class PublicApiCatalogSearchSkill(BaseSkill):
    """Search local catalog of known public APIs. No network calls."""

    def __init__(self) -> None:
        super().__init__(
            name="public_api_catalog_search",
            description="Search local catalog of known public APIs. Params: query (optional).",
            version="1.0.0",
            access_level=AccessLevel.GLOBAL,
            idempotent=True,
        )
    ...
    async def _execute_impl(self, params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
        ...
        catalog_path = base / "data" / "catalog" / "public_api_catalog.json"
        query = (params.get("query") or "").strip().lower()
        if not catalog_path.exists():
            return ActionResult("SUCCESS", "catalog empty", {"entries": [], "count": 0})
        try:
            data = json.loads(catalog_path.read_text(encoding="utf-8"))
            entries = data.get("apis") if isinstance(data, dict) else (data if isinstance(data, list) else [])
            ...
            if query:
                ...
                entries = [{"name": e.get("name"), "description": e.get("description"), "base_url": e.get("base_url"), "docs_url": e.get("docs_url"), "auth": e.get("auth"), "category": e.get("category")} for e in entries if isinstance(e, dict)][:50]
            return ActionResult("SUCCESS", f"count={len(entries)}", {"entries": entries, "count": len(entries)})
        except Exception as e:
...
register_skill(TimeNowSkill())
register_skill(UuidNewSkill())
